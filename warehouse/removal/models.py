...
class RemovalRecord(db.Model):
    """货品下架记录表
    
    Attributes:
        goods_id: 关联商品ID (外键约束不可删除)
        location_id: 来源库位ID (必须存在于locations表)
        quantity: 下架数量 (必须大于0)
        removal_time: 下架时间 (自动记录操作时间)
        reason: 下架原因 (return/damage/transfer)
    ...
    __table_args__ = (
        # 核心业务索引
        db.Index('idx_removal_goods_time', 'goods_id', 'removal_time'),      # 商品维度分析
        db.Index('idx_removal_location_time', 'location_id', 'removal_time'),# 库位维度分析
        db.Index('idx_removal_operator_time', 'operator_id', 'removal_time'),# 操作员效率分析
    ...
    quantity = db.Column(
        ...
        default=1,  # 合理默认值
        info={
            'check': 'quantity > 0',  # 数量校验
            'description': '下架数量（必须>0）'
        }
    )
    ...
    goods = db.relationship(
        'Goods', 
        backref=db.backref('removal_records', lazy='dynamic'),  # 动态反向查询
